...
ENTITY_TIME_REGEX = r'(?P<entity_code>[a-z0-9A-Z\.\_\-]+)/from(?P<from_m>[0-9]{2})(?P<from_y>[0-9]{4})to(?P<to_m>[0-9]{2})(?P<to_y>[0-9]{4})'
...
urlpatterns = patterns('',
    ...
    url(r'^users/edit/(?P<user_id>[0-9]+)$', views.edit_user, name='edit_user'),
    url(r'^users/disable/(?P<user_id>[0-9]+)$', views.disable_user, name='disable_user'),

    url(r'^auto$', ListView.as_view(
            queryset=Provider.objects.order_by('user__first_name', 'user__last_name')[:50],
    ...
    url(r'^static/admin/(?P<path>.*)$',
             ...
             {'document_root': os.path.join(os.path.dirname(os.path.abspath(admin.__file__)), 'media'), 'show_indexes': True}, \
             name='static_admin'),
